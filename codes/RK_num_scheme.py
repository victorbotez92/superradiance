...
def t_step(y_cur,E_cur,tau,all_parameters): #cur stands for current
    
    y_rk1,E_rk1 = rk_t_term(E_cur,y_cur,y_cur,tau,all_parameters)
    y_rk2,E_rk2 = rk_t_term(1/2*(E_cur+E_rk1),1/2*(y_cur+y_rk1),y_cur,tau+all_parameters.dtau/2,all_parameters)
    y_rk3,E_rk3 = rk_t_term(1/2*(E_cur+E_rk2),1/2*(y_cur+y_rk2),y_cur,tau+all_parameters.dtau/2,all_parameters)
    y_rk4,E_rk4 = rk_t_term(E_rk3,y_rk3,y_cur,tau+all_parameters.dtau,all_parameters)
...
def rk_t_term(E_dpt,y_dpt,y_prev,tau,all_parameters):
